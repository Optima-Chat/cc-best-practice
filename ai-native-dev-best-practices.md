...
- **Anthropic 企业版（Claude Enterprise）** 提供更严格的数据隔离和合规保障
- **AWS Bedrock** 上的 Claude 可以在你自己的 AWS 账户内运行，数据不出你的 VPC
- 试点阶段建议选择**非敏感项目**，验证效果后再讨论敏感项目的安全方案

这个问题不回避。对于政府和金融客户的项目，需要根据具体合规要求选择合适的部署方案，这也是试点阶段要一起评估的内容。
...
**依赖 Claude Code 特性的主要是两个：** Dev Skills（按需加载知识文件）和多开的 Agent 模式。随着 AI 编程工具的发展，类似的能力其他工具也会具备。

**降低风险的做法：** 核心资产沉淀在 CLAUDE.md、测试、文档、Issues/PR 里——这些都是标准格式的文件和 GitHub 记录，不绑定任何工具。即使将来换工具，这些资产的价值不会丢失。
...
我今天举的例子都是电商系统，但 10 条实践本身跟业务领域无关。"先学习再做方案""让 AI 写测试""充分利用 Issues 和 PR"——这些是工程方法论，不是电商方法论。
