...
我们做了一个叫 [optima-dev-skills](https://github.com/Optima-Chat/optima-dev-skills) 的项目，里面包含 7 个自定义 Skill，覆盖了日常开发中最常见的运维和调试需求。
...
使用场景：你跟 Claude Code 说"帮我看看 Stage 的 [commerce-backend](https://github.com/Optima-Chat/commerce-backend) 最近 30 分钟有没有报错"，它会自动激活这个 Skill，构造正确的 CloudWatch Logs Insights 查询，拉取日志，分析错误信息，给你总结。
...
这个最短，只有 64 行，因为它主要是把 [commerce-cli](https://github.com/Optima-Chat/commerce-cli) 的使用方式介绍给 Claude Code。让它知道可以通过 CLI 来操作电商系统的各种资源——商品、订单、库存、运费等。
...
### 7. restart-ecs —— 重启 ECS 服务

这个 Skill 让 Claude Code 能够重启 AWS ECS 上的服务。部署后发现问题需要快速回滚，或者服务卡住需要重启，直接跟 Claude Code 说就行，不用登录 AWS 控制台。支持 Stage 和 Prod 两个环境。
...
技术上的实现很简单：NPM 包的 `postinstall` hook 会自动把 7 个 Skill 文件复制到 `~/.claude/` 目录下。Claude Code 启动时会扫描这个目录，发现有可用的 Skills 就会在合适的时候激活它们。
...
一个 NPM 包，7 个 Skill 文件，一千多行文本。投入极小，回报极大。
