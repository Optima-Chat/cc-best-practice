...
> 三个看起来很小的选择，会决定你用 Claude Code 开发的上限。
...
**commerce-cli（660 行）：** 这是我们电商 CLI 工具的 CLAUDE.md，覆盖了 16 个命令模块的完整说明、输出格式约定（结构化 vs --pretty）、错误处理规范、国际化策略、测试要求。660 行，内容非常详尽。

**commerce-backend（521 行）：** 电商后台的 CLAUDE.md，覆盖数据库模型、API 路由表、中间件链、认证授权流程、第三方服务集成（Stripe、ShipStation、Cloudflare R2）。
