...
Optima AI 做跨境电商 AI 平台，核心团队 3 个人，6 个月写了 126 万行代码，30 个仓库，8,105 次 commit。全程用 Claude Code 开发。
...
Claude Code 写的代码你不确定对不对，但测试可以告诉你。让它写测试的成本极低——它很擅长——但带来的质量保障价值极高。我们的 commerce-backend 有 4,600+ 个测试用例，100% 通过率。**测试 + 静态类型检查**，是确保代码质量的两道防线。
...
## 6. [给你的后端写个 CLI](06-cli-first.md)

让 Claude Code 测一遍 API 端点，远比让它跑一条 CLI 命令慢得多，也更容易出错。我们给 commerce-backend 写了完整的 CLI 封装：19 个模块、280+ 条命令。CLI 让 Claude Code 能随意组合测试，而且写 CLI 本身就是 Claude Code 最擅长的事。

## 7. [做团队自己的 Dev Skills](07-dev-skills.md)

把系统的日志、数据库、环境变量、服务重启等操作都做到 Dev Skills 里。我们做了 7 个自定义 Skill，效果是很多功能开发、问题修复，Claude Code 可以全程全自动完成。
...
踩过的坑、UI 开发难点、代码质量、Claude Code vs Codex vs Gemini CLI、数据安全、知识产权、供应商依赖——详见[常见问题文档](12-faq-and-limitations.md)。
